...
export default {
    data: new SlashCommandBuilder()
        .setName('slap')
        .setDescription('Donne une gifle à quelqu\'un (amicalement... ou pas)')
        .addUserOption(option =>
            option.setName('utilisateur')
                .setDescription('La personne à qui tu veux donner une gifle')
                .setRequired(true)),
    async execute(interaction, adminManager, warnManager, guildConfig, sharedConfig, backupToGitHub, reportManager, banlistManager, blockedWordsManager, watchlistManager, telegramIntegration, funCommandsManager) {
        try {
            ...
            const response = await funCommandsManager.executeSlap(interaction, warnManager);
